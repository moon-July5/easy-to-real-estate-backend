...
public class PresubscribeImpl implements PresubscribeService {
    ...
    // 사전 등록 기능
    @Override
    public CommonResponse register(String email){
        if(!isValidEmail(email))
            return responseService.getFailResponse(400, "이메일 형식이 올바르지 않습니다");

        Optional<Presubscribe> customer = repository.findByEmail(email);

        if(!customer.isPresent()){ // 이메일이 존재하지 않으면 이메일 저장
            Presubscribe newCustomer = Presubscribe.builder().email(email).count(1).build();
